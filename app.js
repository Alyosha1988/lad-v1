...
function showStart() {
  brandSub.textContent = "Стартовый аккорд — точка отсчёта гармонии";
  stage.innerHTML = `
    <div class="panel">
      ${renderProgress(0, 5)}
...
function showMove() {
  stage.innerHTML = `
    <div class="panel">
      ${renderProgress(2, 5)}
      <p class="step-label">Шаг 3 из 5</p>
      <h2 class="question">Куда должна вести гармония?</h2>
      ${renderChoices(
        MOVES,
        "moods",
        (m) => `<button type="button" class="choice" data-value="${m.id}">
...
function showPart() {
  stage.innerHTML = `
    <div class="panel">
      ${renderProgress(3, 5)}
      <p class="step-label">Шаг 4 из 5</p>
      <h2 class="question">Какой участок формы?</h2>
      ${renderChoices(
        PARTS,
        "moods",
        (m) => `<button type="button" class="choice" data-value="${m.id}">
...
function showStyle() {
  stage.innerHTML = `
    <div class="panel">
      ${renderProgress(4, 5)}
      <p class="step-label">Шаг 5 из 5</p>
      <h2 class="question">Какой гармонический язык?</h2>
      <p class="hint">«Все семейства» — полный каталог. Остальное — фильтр, но сильные совпадения всё равно пробьются.</p>
      ${renderChoices(
        STYLES,
        "moods",
        (m) => `<button type="button" class="choice" data-value="${m.id}">
  ...
  `;
  bindChoices(() => showResults(), "style");
}

function bindChoices(next, key) {
...
function groupByFamily(ideas) {
  const map = new Map();
  for (const idea of ideas) {
    if (!map.has(idea.family)) map.set(idea.family, []);
    map.get(idea.family).push(idea);
  }
  return map;
}
...
function showResults() {
  const answers = { ...state };
  const { key, ideas } = progressionsFor(answers);
  const mel = melodyFor(answers, key);
  ...
  const scaleBlocks = mel.scales
    .map(
      (s) => `
      <div class="scale-block">
        <p class="scale-name">${s.name}</p>
        <div class="melody-notes">${s.notes.map((n) => `<span class="note">${n}</span>`).join("")}</div>
      </div>`
    )
    .join("");
  ...
  stage.innerHTML = `
    <div class="panel panel-wide">
      ${renderProgress(5, 5)}
      <p class="step-label">Каталог ходов</p>
      ...
      <div class="family">
        ...
        <article class="result">
          <p class="result-why">${mel.tips.join(" ")}</p>
          ${scaleBlocks}
        </article>
      </div>
      <div class="actions">
