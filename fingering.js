...
function renderChordSvg(symbol, voicing, opts = {}) {
  ...
  let marks = "";
  if (base === 1) {
    marks += `<rect x="${padL - 1}" y="${padT - 3}" width="${gridW + 2}" height="3.5" fill="#1d1d1f"/>`;
  } else {
    marks += `<text x="${padL - 5}" y="${padT + gridH / 8 + 3}" fill="#6e6e73" font-size="9" font-family="Manrope,system-ui,sans-serif" text-anchor="end">${base}fr</text>`;
  }

  for (const x of stringXs) {
    marks += `<line x1="${x}" y1="${padT}" x2="${x}" y2="${padT + gridH}" stroke="#86868b" stroke-width="1" opacity="0.85"/>`;
  }
  for (const y of fretYs) {
    marks += `<line x1="${padL}" y1="${y}" x2="${padL + gridW}" y2="${y}" stroke="#86868b" stroke-width="1" opacity="0.65"/>`;
  }
  ...
  for (let s = 0; s < 6; s++) {
    ...
    if (f === FRET_MUTE) {
      ...
      marks += `<circle cx="${x}" cy="${padT - 12}" r="3.8" fill="none" stroke="#0071e3" stroke-width="1.3"/>`;
    } else {
      const relF = f - base + 1;
      if (relF < 1 || relF > showFrets) continue;
      if (barreFret === relF && s > barreFrom && s < barreTo) continue;
      const y = padT + ((relF - 0.5) * gridH) / 4;
      marks += `<circle cx="${x}" cy="${y}" r="5.2" fill="#1d1d1f"/>`;
    }
  }
...
function renderPianoSvg(item, opts = {}) {
  ...
  whites.forEach((m, i) => {
    ...
    svg += `<rect x="${x}" y="${padT}" width="${whiteW - 1.2}" height="${whiteH}" rx="1.5" fill="${on ? "#0071e3" : "#ffffff"}" stroke="#1d1d1f" stroke-width="1"/>`;
    if (on && fingerOf[m]) {
      svg += `<text x="${x + (whiteW - 1.2) / 2}" y="${padT + whiteH - 8}" text-anchor="middle" fill="#ffffff" font-size="10" font-weight="600" font-family="Manrope,system-ui,sans-serif">${fingerOf[m]}</text>`;
    }
  });
  ...
  whites.forEach((m, i) => {
    ...
    if (nextBlack <= end && isBlack(nextBlack)) {
      ...
      svg += `<rect x="${x}" y="${padT}" width="${blackW}" height="${blackH}" rx="1" fill="${on ? "#0071e3" : "#1d1d1f"}" stroke="#000000" stroke-width="0.5"/>`;
      if (on && fingerOf[nextBlack]) {
        svg += `<text x="${x + blackW / 2}" y="${padT + blackH - 6}" text-anchor="middle" fill="#ffffff" font-size="9" font-weight="600" font-family="Manrope,system-ui,sans-serif">${fingerOf[nextBlack]}</text>`;
      }
    }
  });
